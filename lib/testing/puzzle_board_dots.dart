...
class PuzzleBoardDots extends StatefulWidget {
  final Size size;
  const PuzzleBoardDots({Key? key,required this.size}) : super(key: key);

  @override
  PuzzleBoardDotsState createState() => PuzzleBoardDotsState();
}

class PuzzleBoardDotsState extends State<PuzzleBoardDots> {
  List<int> initial=[];
  List<int> goalState=[];
  List<Tile> tiles=[];
          ...
          return AnimatedPositioned(
            ...
            child: tiles[index].value==0?const SizedBox():GestureDetector(
              ...
              child: SizedBox(
                ...
                child: TileWidget(size: tiles[index].size),
              ),
            ),
          );
        })
